...
`/v1` 智能体的领域工具只读。内部还会使用 deep-agents 框架工具（`write_todos`、`task`、`eval`、文件系统工具）。这些运行出现在响应对象的**顶层 `tool_steps` 扩展**中（不作为 `output` item，以免官方 SDK 解析未知类型），流式下则通过 [`response.mirobody_tool_call` 旁路事件](/zh/api-reference/streaming#服务端工具旁路通道)返回。
...
| `tool_choice` | `"auto"`（默认）、`"none"`（本轮禁用**所有**工具，含内置工具 —— 见 [backbone 模式](/zh/api-reference/backbone-mode)）或指名某个客户端工具。其它取值 → `400 unsupported_parameter`。 |

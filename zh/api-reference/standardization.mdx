---
title: "标准化"
description: "结构化读数如何变成带编码、单位归一并带 FHIR 镜像的数据。"
---

健康数据到达时是杂乱的 —— "血糖(空腹)"、"FBG"、"Glucose, fasting" 是同一项检测的三副面孔，单位同样五花八门。Mirobody 会对通过 [`POST /v1/data`](/zh/api-reference/data) 写入或由 [`POST /v1/extract`](/zh/api-reference/extract) 存储的每条**结构化读数**做标准化，让智能体和你的查询看到一份连贯、带编码的数据集。

[`POST /v1/files`](/zh/api-reference/files) 是独立的文件存储与文本抽取界面。上传文件不会创建结构化读数；需要把报告数值标准化并存储时，请将其 `file_key` 传给 `/v1/extract`。
...
你发送给 `/v1/data` 的内容（或 `/v1/extract` 从报告中读到的内容）与结构化存储中的结果：
...
| 值 | `"97 mg/dL"`（一个字符串） | `parsed_value: "97"`，`parsed_unit: "mg/dL"`（UCUM） |

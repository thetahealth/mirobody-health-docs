...
它的数据为非生产、可能被随时重置，因此切勿在其中放置真实的终端用户数据。生产环境（`https://mirobody-api.thetahealth.ai/v1`）与**中国**集群（`https://mirobody-api.thetahealth.cn/v1`）均在筹备中 —— 完整的主机矩阵与上线状态见[区域](/zh/api-reference/regions/overview)。

...
[Answers API](/zh/api-reference/chat) 是一个**封闭的 grounded completion** —— 问题进、有证据的回答出，没有额外开关。这个形状恰好就是外层 agent 想要的工具：你的 agent（跑在任何模型、任何框架上）保有编排权，把*"这位用户的真实健康记录说明了什么？"*委托给 Mirobody。

---
title: "POST /api/chat"
description: "流式对话接口（SSE）。核心入口，需要 BYOK header。"
---

<Warning>
**此接口需要 LLM 凭证。** 调用时需在 `Authorization` 之外额外携带 `X-LLM-Provider` 与 `X-LLM-Api-Key` header，详见 [BYOK 协议](/zh/cloud/byok/index)。
</Warning>

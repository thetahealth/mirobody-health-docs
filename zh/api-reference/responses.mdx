...
`usage.billed_tokens`（`{input, output, total}`）是 **provider-true** 汇总 —— 平台实际计量（`api_usage_record`）并计价的数字。成本核算用它即可，不必再自己把 visible + system 相加。Agent API 与 [Answers API](/zh/api-reference/chat) 两面都有。成本 = `billed_tokens` × [档位单价](/zh/api-reference/models)。

...
| `tools` | array | 客户端 **function** 工具（`{"type": "function", "name", "description", "parameters"}`；completions 嵌套形式也接受）。最多 64 个；名称需匹配 `[a-zA-Z0-9_-]{1,64}` 且不得与内置工具重名。见 [Function calling](/zh/api-reference/function-calling)。 |

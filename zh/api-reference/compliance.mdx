...
<AccordionGroup>
  ...
  <Accordion title="用户权利" icon="key">
    通过 API 可按数据主体导出数据（[结构化数据](/zh/api-reference/data) / [文件](/zh/api-reference/files)）；有时限的[留存](/zh/api-reference/overview#数据留存)会自动使数据过期（会话数据上限为 24 小时）。`/v1` 内建自助擦除：`DELETE /v1/data`（记录）、`DELETE /v1/files/{file_key}`（文件）、`DELETE /v1/responses/{id}`（存储的对话）、`DELETE /v1/sessions/{id}`（会话范围数据），以及 `DELETE /v1/subjects/{user}` —— 一次性擦除该 Subject 的**全部数据**（记录、文件、对话与身份映射）。如需注销账户，请联系 [Mirobody Support](mailto:developer@thetahealth.ai)。
  </Accordion>
</AccordionGroup>

...
任何以 `gAAAA` 开头的字符串值都会被当作 Fernet token，在设置了 `CONFIG_ENCRYPTION_KEY` 时于加载时解密（`config.yml` 和远程拉取都适用）。加密由运维方自己完成：没有任何机制会改写静态的明文 —— 请用 `fernet` CLI（`cli/fernet.cpp`：`fernet genkey` / `fernet encrypt`）加密每个密钥，再把 `gAAAA…` 密文粘贴进配置。此外，**键名**包含 `_KEY`、`_PASSWORD`、`_SECRET`、`_TOKEN`（及类似）的值在配置摘要输出时会被打码。

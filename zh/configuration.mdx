---
title: "配置"
description: "Mirobody 完整配置指南：OAuth、AI 提供商、Agent、MCP 暴露、存储与安全。"
---

## 配置概览

Mirobody 使用分层 YAML 配置。本指南覆盖所有可配项及其设置方式。任何键名包含 `_KEY`、`_PASSWORD`、`_PASS`、`_PWD`、`_SECRET`、`_SK`、`_TOKEN` 的字段，会在首次加载时使用你的 `CONFIG_ENCRYPTION_KEY` 自动加密。
...
Mirobody 内置 **三种** Agent：

- **DeepAgent** —— 单模型工具编排，默认选择；需要 `OPENROUTER_API_KEY`
- **MixAgent** —— 两阶段融合（强模型编排 + 便宜模型回复），兼顾成本与质量
- **BaseAgent** —— 不调工具的直连 LLM 对话，适合低延迟 Q&A

### DeepAgent 配置

DeepAgent 支持多个 LLM 提供商，推荐用于复杂任务。**`OPENROUTER_API_KEY` 是它启动的必要条件**。
...
MixAgent 跑两个 phase —— 强模型负责工具编排和数据收集，便宜模型基于收集到的上下文写出最终回复。适合编排阶段需要更强推理、回复阶段不必那么贵的场景：

```yaml config.localdb.yaml
# 同 DeepAgent 一样的 key 命名约定
PROVIDERS_MIX:
  orchestrator:
    llm_type: openai
    api_key: OPENROUTER_API_KEY
...
BaseAgent 针对轻量级操作进行了优化：

```yaml config.localdb.yaml
# 用于 BaseAgent 的 API 密钥
GOOGLE_API_KEY: '您的_google_api_key'           # 用于 Gemini 模型
...
<Steps>
  <Step title="注册为 Garmin 开发者">
    1. 访问 <a href="https://developer.garmin.com/">Garmin Connect 开发者门户</a>
    2. 使用您的 Garmin 账号登录
...
<Steps>
  <Step title="注册为 Whoop 开发者">
    1. 访问 <a href="https://developer.whoop.com/">Whoop 开发者门户</a>
    2. 创建开发者账号
    3. 创建一个新应用
    4. 设置所需的权限范围 (Scopes)：
       - `offline` (用于刷新令牌)
...
<Steps>
  <Step title="获取 API 密钥">
    1. 在 <a href="https://platform.openai.com/">OpenAI 平台</a> 注册
    2. 导航至 API Keys 部分
    3. 创建新的 API 密钥
  </Step>
</Steps>
...
<Steps>
  <Step title="获取 OpenRouter API 密钥">
    1. 在 <a href="https://openrouter.ai/">OpenRouter</a> 注册
    2. 导航至 Keys 部分
    3. 创建新的 API 密钥
  </Step>
</Steps>
...
<Steps>
  <Step title="获取 API 密钥">
    1. 在 <a href="https://console.anthropic.com/">Anthropic Console</a> 注册
    2. 导航至 API Keys
    3. 创建新的 API 密钥
  </Step>
</Steps>
...
<Info>
**`MCP_PUBLIC_URL` 就是 HTTP Remote MCP 的开关。** 把它设成一个公网 HTTPS URL（比如 `https://yourdomain.com` 或者隧道地址 `https://abc123.ngrok.io`），你的 MCP 服务器立即可以被 ChatGPT Apps、远程模式的 Claude Desktop、以及任何其他 MCP 客户端在 `<MCP_PUBLIC_URL>/mcp` 上访问。详见 [ChatGPT Apps](/zh/tools/chatgpt-apps)。
</Info>
...
<Tabs>
  <Tab title="Google OAuth">
    ...
    从 <a href="https://console.cloud.google.com/">Google Cloud Console</a> 获取凭据。
  </Tab>
  ...
  <Tab title="Apple 登录">
    ...
    从 <a href="https://developer.apple.com/">Apple 开发者门户</a> 获取凭据。
  </Tab>
</Tabs>
...
内置的 `execute` 工具用 [E2B](https://e2b.dev) 沙箱跑 shell / Python。不填 `E2B_API_KEY` 时，`execute` 会退化为安全的 no-op stub，不影响系统其它功能；填上 key 即可解锁真实代码执行。PostgreSQL 持久化的文件操作（`write_file`、`read_file` 等）与沙箱保持同步 —— Agent 写下的文件下一轮 `execute` 能直接读到。

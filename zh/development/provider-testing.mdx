...
独立 CLI 从遗留的 `MIROBODY_VENDOR_<ID>_*` 环境变量读取凭据（它自己的 `from_env()` 约定）；服务器本身则用简洁的 `<ID>_*` 键配置（`FITBIT_CLIENT_ID` 等）—— 参见[使用 Provider](/zh/providers/using-providers)。
...
- 一个隧道服务，如 **ngrok**（在本地测试期间也便于接收入站 **webhook**）。
...
<AccordionGroup>
  ...
  <Accordion title="Token 生命周期（exchange_code / refresh）" icon="key">
    用一个新鲜的授权码调用 `exchange_code(code, redirect_uri)`，应返回一个 `TokenSet`（access token、若 vendor 签发则含 refresh token、`expires_in`）；`refresh(refresh_token)` 应返回一组新 token。错误或已用过的授权码 → 一个携带 vendor 错误响应体的清晰 `VendorError`。没有 refresh 授权的 vendor（Polar —— 其 token 不过期）应抛出其文档中说明的原因。端到端层面，这对操作由 `POST /vendors/{id}/bind/verify`（交换授权码并加密存储用户 token）以及 `fetch` 在 token 过期时的自动刷新来覆盖。
  </Accordion>

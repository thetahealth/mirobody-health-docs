...
<Steps>
  ...
  <Step title="构建">
    ...
    ./build/mirobody      # 运行（读取 ./config.yml）
    ```

    `build.sh` 可按任意顺序接收一个架构和/或后端选择器，另加 `clean`：
    ```bash
    ./build.sh sqlite         # SQLite 后端 -> ./build-sqlite
    ./build.sh mysql          # MySQL 后端 -> ./build-mysql
...
其他构建产物：`build-shared.sh`（`libmirobody` C-ABI 共享库）、`build-python.sh`（Python wheel，经 pybind11 **从源码构建**），以及 Android（Gradle + NDK）/ iOS（`mirobody.xcframework`）目标。还有一个多阶段 `Dockerfile`。

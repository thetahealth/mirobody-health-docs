...
Mirobody 的测试是**用 [Catch2 v3](https://github.com/catchorg/Catch2) 编写的 C++ 单元测试**，位于 [`tests/`](https://github.com/thetahealth/mirobody/tree/main-v2/tests) 下，编译进单个 `mirobody_tests` 二进制。它们默认构建，并由 CTest 发现。
...
<Note>
在 Windows 上，二进制为 `build\tests\mirobody_tests.exe`。构建目录名随你的后端选择器而定（如 `build-sqlite\tests\mirobody_tests.exe`）。
</Note>
...
在匹配的 `tests/<module>/` 目录下新增一个 `*_test.cpp`，然后在 [`tests/CMakeLists.txt`](https://github.com/thetahealth/mirobody/blob/main-v2/tests/CMakeLists.txt) 的 `add_executable(mirobody_tests ...)` 源文件列表中登记它。
...
CI 在推送时运行构建与测试。流水线定义在仓库的 CI 配置（`.gitlab-ci.yml`）中。

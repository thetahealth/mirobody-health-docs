...
<AccordionGroup>
  ...
  <Accordion title="凭据" icon="key">
    一个 API key 或 OAuth `client_id` / `client_secret`，通过按 vendor 的 `<ID>_*` 配置键（如 `ACME_CLIENT_ID`）送达 `VendorConfig`，可写在 `config.yml` 中或用同名环境变量设置。
  </Accordion>
</AccordionGroup>
...
<Steps>
  ...
  <Step title="注册它">
    在 [`src/health/vendor/registry.cpp`](https://github.com/thetahealth/mirobody/blob/main-v2/src/health/vendor/registry.cpp) 中声明工厂并添加一行。
  </Step>
</Steps>
...
| `exchange_code(code, redirect_uri)` | 该源有 OAuth2 授权码 token 端点 —— 用授权码换取 `TokenSet` | 桩 |
| `refresh(refresh_token)` | 该源签发会过期的 token 且有 refresh 授权 —— 用 refresh token 换新 `TokenSet` | 桩（如 Polar 的 token 永不过期） |

对于标准的 RFC 6749 token 端点，不要手写 POST：共享的辅助函数 `oauth2_token_request(...)`（[`src/health/vendor/oauth2.hpp`](https://github.com/thetahealth/mirobody/blob/main-v2/src/health/vendor/oauth2.hpp)）负责发送表单编码请求（client 凭据放请求体或走 HTTP Basic）并解析 `{access_token, refresh_token?, expires_in?}` 响应 —— Fitbit、Dexcom、Oura、Whoop、Polar 的 `exchange_code` / `refresh` 都经它实现（Withings 使用自己的响应信封）。
...
服务器从中央配置构建 `VendorConfig`，使用简洁的按 vendor 键，可写在 `config.yml` 中或用同名环境变量设置（参见 `config.example.yml`）：

```
ACME_API_KEY / ACME_CLIENT_ID / ACME_CLIENT_SECRET / ACME_BASE_URL
```

当存在一个 API key，或一对 client id+secret 时，`VendorConfig::configured()` 为真。（旧的 `MIROBODY_VENDOR_<ID>_*` 环境变量约定只保留在 `VendorConfig::from_env()` 中，仅供独立的 `vendor` CLI 自行使用。）

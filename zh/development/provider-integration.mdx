...
对于标准的 RFC 6749 token 端点，不要手写 POST：共享的辅助函数 `oauth2_token_request(...)`（[`src/health/vendor/oauth2.hpp`](https://github.com/thetahealth/mirobody/blob/main/src/health/vendor/oauth2.hpp)）负责发送表单编码请求（client 凭据放请求体或走 HTTP Basic）并解析 `{access_token, refresh_token?, expires_in?}` 响应 —— Fitbit、Dexcom、Oura、Whoop、Polar 的 `exchange_code` / `refresh` 都经它实现（Withings 使用自己的响应信封）。

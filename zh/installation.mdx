...
<Info>
没有 `pip install mirobody`，也没有 Python 虚拟环境流程 —— 那是上一代引擎。你用 `./build.sh`（Windows 上用 `build.cmd`）构建原生二进制，或者用 Docker。
</Info>
...
<AccordionGroup>
  ...
  <Accordion title="Python wheel —— 从源码构建（非 PyPI）" icon="python">
    `mirobody._mirobody` 是一个 CPython 扩展（一个包装 `mirobody::Server` 的 pybind11 模块），由 scikit-build-core 打包成 `.whl` —— 你**从源码构建它**，并没有从 PyPI `pip install mirobody` 这回事：
...
<Note>
Agent 和 MCP 工具**在编译期自注册**：把一个 `.cpp` 丢进 `res/agents/` 或 `res/mcp_tools/` 再重新构建即可。没有运行时插件目录，也没有 `mirobody/pub/...` 这样的 Python 包。
</Note>

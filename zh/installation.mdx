...
<Tabs>
  <Tab title="Linux / WSL / macOS">
    ...
    Token：`pg` / `postgresql`、`legacy` / `pg_legacy`、`mysql`、`sqlite`、`duckdb`、`ck` / `clickhouse`（省略即用 `POSTGRESQL` 默认）。每个后端构建到各自的 `build[-<backend>]` 目录，因此多个可以共存。
  ...
  <Tab title="Windows (MSVC + vcpkg)">
    ...
    vcpkg 会自动从 [`vcpkg.json`](https://github.com/thetahealth/mirobody/blob/main-v2/vcpkg.json) 拉取原生依赖 —— 无需手动安装。
...
```bash
./build/mirobody                  # 默认读取 ./config.yml
...
│   ├── llm/                # 流式 LLM 客户端（OpenAI、Gemini；chat / realtime / embeddings）
...
├── res/                    # 由二进制编译/托管的内置资源
│   ├── agents/baseline.cpp     # 唯一的 agent，编译期自注册
...
<Steps>
  ...
  <Step title="日志">
    二进制把日志打到 stderr；用进程管理器捕获。Docker 下用 `docker logs <container>`。
  </Step>
</Steps>
...
<AccordionGroup>
  <Accordion title="响应未压缩 / libwebsockets 版本" icon="circle-exclamation">
    发行版的 `libwebsockets-dev` 包在编译时**没有**开启 `LWS_WITH_HTTP_STREAM_COMPRESSION`，所以所有响应都不压缩。对带宽敏感的部署，请从源码构建 libwebsockets 并开启 `-DLWS_WITH_HTTP_STREAM_COMPRESSION=ON -DLWS_WITH_ZLIB=ON`，版本须 **≥ 4.3.4**（CVE-2025-1866 是该压缩路径中的越界缺陷，4.3.4 已修复）—— Docker 镜像正是这么做的，锁定在 `v4.3.10`。参见 `docs/BUILDING.md`。
  </Accordion>

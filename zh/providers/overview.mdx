---
title: "Provider 概览"
description: "了解 Mirobody 的 Provider 系统及其与健康设备 API 的集成方式"
---

## 什么是 Provider？

Provider 是将 Mirobody 与外部健康和健身设备 API 集成的插件模块。每个 Provider 负责处理 OAuth 身份验证、数据获取以及将数据转换为标准化格式。

## Provider 架构

Mirobody 使用可扩展的 Provider 架构，使添加新的健康设备集成变得非常容易：
...
Mirobody 会从配置的目录中自动发现并加载 Provider：

...
- **`TokenSet`** 携带一次 OAuth2 token 端点调用返回的 `access_token` / `refresh_token` / `expires_in`。各 OAuth 设备品牌的 `exchange_code` / `refresh` 通过共享的辅助函数实现，即 [`src/health/vendor/oauth2.hpp`](https://github.com/thetahealth/mirobody/blob/main/src/health/vendor/oauth2.hpp) 中的 `oauth2_token_request(...)`（Withings 使用自己的响应信封；Polar 的 token 不过期，其 `refresh` 直接抛错）。

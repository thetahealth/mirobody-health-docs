...
在 Mirobody 中，**vendor** 是针对某个健康数据源的 C++ 客户端，在 `src/health/vendor/` 下一源一文件。每个 vendor 都实现同一个抽象接口 —— [`src/health/vendor/vendor.hpp`](https://github.com/thetahealth/mirobody/blob/main-v2/src/health/vendor/vendor.hpp) 中的 `vendor::Vendor` —— 因此引擎的其余部分能通过同一套契约与可穿戴品牌、B2B 聚合器、医院 EHR 对话，并在运行时按 **id** 挑选具体客户端。
...
<CardGroup cols={2}>
  ...
  <Card title="设备品牌" icon="watch">
    `device/` —— 消费级可穿戴品牌的直连客户端：**Fitbit、Withings、Garmin、Dexcom、Oura、Whoop、Polar**
  </Card>
...
拥有各自 OAuth REST API 的设备*品牌*有两条接入路径：**通过聚合器**（Terra、Validic、Thryve、Rook、Spike），或使用专属的**第一方客户端**。目前有七个品牌自带直连客户端 —— **Fitbit、Withings、Garmin、Dexcom、Oura、Whoop、Polar** —— 每个都针对品牌的公开 API 实现了真实的 OAuth 流程与 `fetch`，供不想经聚合器的部署直接获取品牌数据。没有客户端的品牌仍只能经聚合器接入，除非有人为其添加一个。
...
- **`TokenSet`** 携带一次 OAuth2 token 端点调用返回的 `access_token` / `refresh_token` / `expires_in`。各 OAuth 设备品牌的 `exchange_code` / `refresh` 通过共享的辅助函数实现，即 [`src/health/vendor/oauth2.hpp`](https://github.com/thetahealth/mirobody/blob/main-v2/src/health/vendor/oauth2.hpp) 中的 `oauth2_token_request(...)`（Withings 使用自己的响应信封；Polar 的 token 不过期，其 `refresh` 直接抛错）。
...
具体的 vendor 类永远不会在其自身的翻译单元之外被命名。调用方只能通过注册表访问它们（[`src/health/vendor/registry.hpp`](https://github.com/thetahealth/mirobody/blob/main-v2/src/health/vendor/registry.hpp)）：
...
vendor 的 `VendorConfig` 由服务器从中央配置填充，使用简洁的按 vendor 键 —— 可写在 `config.yml` 中，也可用同名环境变量设置（参见 `config.example.yml`）：
...
`<ID>` 是 vendor id 的大写形式（如 `HUMAN_API`、`FITBIT`、`OURA`）。对于主机随部署而异的 vendor（自托管或需合同授权，如通用 `ehr` 客户端），`base_url` 是**必填**的 —— 客户端会拒绝执行，而不是猜测一个主机。（旧的 `MIROBODY_VENDOR_<ID>_*` 环境变量约定已是遗留用法，只有独立的 `vendor` CLI 仍自行读取它。）
...
<Warning>
`/api/providers` 是一个**不同的**端点 —— 它为模型选择器列出聊天用的 **LLM providers**（OpenAI、Gemini 等），而非健康 vendor。健康数据源即本文所述的 `vendor::Vendor` 客户端。
</Warning>

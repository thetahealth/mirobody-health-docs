...
<AccordionGroup>
  ...
  <Accordion title="用户权利" icon="key">
    通过 API 可按数据主体导出数据（[结构化数据](/zh/api-reference/data) / [文件](/zh/api-reference/files)）；结束会话会清除其数据（[sessions](/zh/api-reference/sessions)）。如需注销账户或提出完整的数据擦除请求，请联系 [Mirobody Support](mailto:developer@thetahealth.ai)。
  </Accordion>
</AccordionGroup>

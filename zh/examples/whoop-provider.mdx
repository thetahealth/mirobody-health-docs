...
token 交换在客户端内部实现：`exchange_code` / `refresh` 通过共享的 `oauth2_token_request(...)` 辅助函数（[`src/health/vendor/oauth2.cpp`](https://github.com/thetahealth/mirobody/blob/main-v2/src/health/vendor/oauth2.cpp)）POST 到 `{base}/oauth2/token`，以保密客户端身份用 HTTP Basic 认证。所得的 access token 在 `fetch` 时作为 `config.api_key` 携带。

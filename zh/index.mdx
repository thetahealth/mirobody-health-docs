...
<CardGroup cols={2}>
  ...
  <Card title="托管 API 平台" icon="cloud" href="/zh/cloud">
...
<AccordionGroup>
  ...
  <Accordion title="一个轻量、会用工具的 Agent" icon="robot">
    只有一个 Agent：它按**提供商**名挑一个 LLM 客户端，构建系统提示词，把这一轮流式返回 —— 并按需调用内置的 **MCP 工具**（读文件、渲染图表、家庭健康、记忆）。早期版本曾提供独立的 Deep / Mix / Base 三种 Agent，如今已收敛为这一个。
  </Accordion>

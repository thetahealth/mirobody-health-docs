...
`CONFIG_ENCRYPTION_KEY` 只做两件事：已经以 Fernet 密文（`gAAAA…`）存储的配置值会在**加载时透明解密**；看起来像密钥的值在配置摘要输出时会被**打码**。没有任何机制会在写入时自动加密明文 —— 需要你自己用 `fernet` CLI（`cli/fernet.cpp`：`fernet genkey` / `fernet encrypt`）预先加密每个密钥，把 `gAAAA…` 密文粘贴进 `config.yml`。照此操作后，只要密钥留在环境里，就可以提交加密后的 `config.yml`。像 `PG_ENCRYPTION_KEY` 这类引导性密钥请放在环境变量或密钥管理器里。
...
- **水平** —— 在负载均衡器后跑多个实例；设置 `REDIS_HOST` 让它们共享缓存状态。所有实例都连到那一个配置好的 PostgreSQL（`PG_HOST` + 连接池）—— 引擎没有只读副本路由，数据库成为瓶颈时请扩展数据库本身。*（只要每个连接在其生命周期内保持在同一实例上，SSE/WebSocket 对话路由就不需要粘性会话 —— 请对照你负载均衡器的连接处理方式核实。）*

...
Mirobody 运行**一个简单的 Agent** —— "Baseline"，定义在 [`res/agents/baseline.cpp`](https://github.com/thetahealth/mirobody/blob/main-v2/res/agents/baseline.cpp)。它刻意保持极简：没有 Agent 循环，没有 planner，没有中间件。每一轮它会：
...
工具是 **[`res/mcp_tools/`](https://github.com/thetahealth/mirobody/tree/main-v2/res/mcp_tools) 中的 C++ 文件**。每个文件声明一个 `Tool`（名称、描述、一个 `auth` 标志、一张小小的 `Param` 表以及一个 handler），并在编译时自注册。`CMakeLists.txt` 会把该目录下的每个 `.cpp` 都 glob 进构建（带 `CONFIGURE_DEPENDS`），因此**只需丢入一个新文件并重新构建即可** —— 无需手动注册，无需手写 JSON schema。
...
| `family_health` | ✅ | 读取用户或已共享照护圈成员的近期 FHIR `Observation` |
...
支持的方法包括 `tools/list` 和 `tools/call`。本地开发时，把端口打通隧道（例如 `ngrok http 8080`），远端客户端就能访问 `/mcp` —— 它们直接连公网 URL，MCP / OAuth 服务会从每个请求的 `Host` 头推导对外公布的 URL。参见 [MCP 集成](/zh/tools/mcp-integration) 连接 Claude 和 Cursor。

...
<Note>
**没有 "deep agents" 特性。** 早期草稿曾描述过任务规划 / todos、任务子 Agent、虚拟文件系统、沙箱执行以及 Agent Skills 注册表。**这些在 C++ 引擎（Mirobody）中都不存在。** 也不再有单独的 Deep / Mix / Base 之分 —— 它们已合并为这一个 Agent。你选择的是一个 **provider**（一个模型），而不是 Agent *类型*。
</Note>
...
<Info>
这是一个**编译期**机制。C++ 引擎中没有 Python 工具加载器，也没有 `mirobody/pub/tools/` 目录 —— 那些是旧版 Python 服务器的一部分。
</Info>

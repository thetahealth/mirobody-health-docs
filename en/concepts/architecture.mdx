...
Discovery → OAuth link → periodic pull → save raw (`th_raw_data`) → normalize to `StandardPulseData` → write (`th_series_data`) → aggregate to daily summaries → insights + indicator search.

...
<Info>
There is no `docker compose` bundle for Postgres/Redis and no `deploy.sh` — those belonged to the old Python engine. You build one image and run it against your own database (and, optionally, Redis).
</Info>

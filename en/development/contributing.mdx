...
Other build outputs: `build-shared.sh` (the `libmirobody` C-ABI shared library), `build-python.sh` (the Python wheel, built **from source** via pybind11 — not `pip install mirobody`), and the Android (Gradle + NDK) / iOS (`mirobody.xcframework`) targets. There's also a multi-stage `Dockerfile`.

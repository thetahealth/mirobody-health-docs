...
For standard RFC 6749 token endpoints, don't hand-roll the POST: the shared helper `oauth2_token_request(...)` in [`src/health/vendor/oauth2.hpp`](https://github.com/thetahealth/mirobody/blob/main/src/health/vendor/oauth2.hpp) does the form-encoded request (body or HTTP Basic client auth) and parses the `{access_token, refresh_token?, expires_in?}` response — Fitbit, Dexcom, Oura, Whoop, and Polar all implement `exchange_code` / `refresh` through it (Withings has its own envelope).

...
Mirobody's tests are **C++ unit tests written with [Catch2 v3](https://github.com/catchorg/Catch2)**, living under [`tests/`](https://github.com/thetahealth/mirobody/tree/main/tests) and compiled into a single `mirobody_tests` binary. They are built by default and discovered by CTest.

<Info>
This is a C++ suite, not `pytest`. (Some Python `.pyc` files linger in `tests/` from the old engine, but the live tests are the `*_test.cpp` files.)
</Info>
...
| **LLM clients** | `tests/llm/*` — SSE parser, OpenAI / Gemini / MiroThinker chat, realtime / live |
...
Add a `*_test.cpp` under the matching `tests/<module>/` directory, then list it in [`tests/CMakeLists.txt`](https://github.com/thetahealth/mirobody/blob/main/tests/CMakeLists.txt) in the `add_executable(mirobody_tests ...)` sources.

...
Billing is metered per **actual (provider-true) token usage** — the [console's Usage page](https://platform.mirobody.ai/developer/usage) and each response's `usage.billed_tokens` are the source of truth for consumption and cost. Self-hosting the open-source engine is free software; you pay only for your own infrastructure and LLM keys.
...
<Note>
The values above are from the **global** cluster (`api.mirobody.ai`). The **China** cluster (`api.mirobody.cn`) exposes the same fields, but its tiers are priced differently — always read `pricing` from the cluster you call rather than hard-coding numbers.
</Note>

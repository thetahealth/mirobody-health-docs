...
Its data is non-production and may be reset at any time, so never put real end-user data there. Production (`https://mirobody-api.thetahealth.ai/v1`) and the **China** cluster (`https://mirobody-api.thetahealth.cn/v1`) are provisioning — see [Regions](/en/api-reference/regions/overview) for the full host matrix and live status.

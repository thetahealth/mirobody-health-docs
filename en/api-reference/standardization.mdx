...
description: "How structured readings become coded, unit-normalized, FHIR-mirrored data."
---

Health data arrives messy — "血糖(空腹)", "FBG", "Glucose, fasting" are the same measurement in three costumes, with units to match. Mirobody standardizes every **structured reading** written through [`POST /v1/data`](/en/api-reference/data) or stored by [`POST /v1/extract`](/en/api-reference/extract), so the agent and your queries see one coherent, coded dataset.

[`POST /v1/files`](/en/api-reference/files) is a separate storage and text-extraction surface. Uploading a file does not create structured readings; pass its `file_key` to `/v1/extract` when you want report values standardized and stored.

## The pipeline

```text
document text / structured record
   │
...
What you send to `/v1/data` (or what `/v1/extract` reads from a report) vs. what the structured store holds:
...
| Value | `"97 mg/dL"` (one string) | `parsed_value: "97"`, `parsed_unit: "mg/dL"` (UCUM) |

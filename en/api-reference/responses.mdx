...
`usage.billed_tokens` (`{input, output, total}`) is the **provider-true** total — the number the platform actually meters (`api_usage_record`) and prices. Use it for cost accounting; you no longer need to add visible + system tokens yourself. It's present on both the Agent API and the [Answers API](/en/api-reference/chat). Cost = `billed_tokens` × the [model tier's rate](/en/api-reference/models).

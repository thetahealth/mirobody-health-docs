...
| --- | --- | --- |
| `records` | array | Each record `{indicator, value, unit?, time?, end_time?, source?}`. **Max 500 records per request.** `end_time` (optional, ISO time) marks the end of an episode — sleep, workouts — whose start is `time`. |

...
<Note>
**Every write runs standardization, not just storage.** Each value/unit is UCUM-parsed and the indicator name is matched to LOINC deterministically (curated corpus + embedding — no LLM guessing). A low-confidence match stays uncoded. See [Standardization](/en/api-reference/standardization). To preview that pipeline without writing anything, use [`POST /v1/extract`](/en/api-reference/extract) with `store=false`.
</Note>
...
The response is an OpenAI-style list, newest first. `value` is the **human-readable string** (e.g. `"97 mg/dL"`); the machine-readable fields ride alongside:

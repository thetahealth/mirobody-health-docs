...
<Warning>
`store=true` is not idempotent. Repeating the same extraction can write duplicate readings. Use the default dry-run to inspect a document first, and avoid blind retries after a timeout.
</Warning>
...
**Purely subjective entries (journaling) use the Agent API — no dedicated endpoint is needed.** Send each entry as a single-turn [`POST /v1/responses`](/en/api-reference/responses) with `store: true`, a unique `session_id` such as `journal-{entry_id}`, `builtin_tools: "none"`, and short-acknowledgement `instructions`. Background consolidation can derive quantifiable indicators and durable memory from the stored turn. With one response per `session_id`, deleting that response also retracts its conversation-derived memory; already extracted readings remain until deleted through `/v1/data`. See [Journaling](/en/api-reference/state-and-memory#journaling) for the exact lifecycle.

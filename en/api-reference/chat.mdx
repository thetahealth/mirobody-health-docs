...
# resp.choices[0].message.reasoning_content     # deep-thinking (CoT) detail
# resp.choices[0].message.reasoning_steps        # tool-call trace
# resp.choices[0].message.tool_steps            # search / fetch / eval detail
```
...
<Note>
**Sampling parameters.** For OpenAI compatibility the request accepts `max_tokens`, `n`, `temperature`, `top_p`, `stop`, and `seed`, but the agent governs its own generation: `max_tokens` is **not enforced** (the agent controls output length), `temperature` / `top_p` / `stop` / `seed` are accepted but **ignored**, and `n` must be `1` — any other value returns `400`.
</Note>
...
```jsonc
{
  ...
  "choices": [{
    ...
    "message": {
      ...
      "reasoning_content": "Let me pull the last 30 days of fasting glucose and compute the trend ...",  // deep-thinking (CoT), optional
...
| `message.reasoning_content` | The model's **deep-thinking** (thinking / CoT) detail. |
| `message.reasoning_steps[]` | The **tool-call** trace — which tools the gather phase invoked (`{type:"tool_call", tool}` entries). |
...
`reasoning_content`, `reasoning_steps`, and `tool_steps` are optional, additive channels — clients that only read `message.content` keep working.

<Note>
`tool_steps[].arguments` is currently always `null` — the built-in agent does not surface tool arguments; rely on `name` + `result`.
</Note>

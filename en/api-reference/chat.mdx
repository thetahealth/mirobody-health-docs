...
| `session_id` | string | Mirobody extension. Tags the call for [session-scoped cleanup](/en/api-reference/sessions). For real multi-turn state, use the [Agent API](/en/api-reference/responses). |
...
The hosted agent is built on **LangChain [deep agents](https://docs.langchain.com/oss/python/deepagents/overview)**: beyond plain tool-calling it can **plan** multi-step work (`write_todos`), **delegate** to sub-agents (`task`), run **code eval** for analysis, and keep a **virtual filesystem** (your uploads in `/uploads`, past uploads in `/library`). You don't invoke any of this — the agent does, against the Subject's data.
...
The deep-agents **harness tools** — `write_todos`, `task`, `eval`, and the filesystem tools (`ls` / `read_file` / …) — are used internally and are **not** listed in `tool_names`. `ask_user` exists for the consumer product but is **disabled on the `/v1` surfaces** (an API caller has no widget to answer it). The public set can vary by cluster; call `GET /api/health` for the live `tool_names`. Files you upload via [`/v1/files`](/en/api-reference/files) land in `/uploads`, and the agent reads them automatically.

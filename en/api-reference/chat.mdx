...
| `message.reasoning_steps[]` | Legacy structured trace of the gather phase: `{type, thought/tool}` entries, kept for back-compat. Prefer `reasoning_content` (free text) and `tool_steps` (tool calls) for new code. |

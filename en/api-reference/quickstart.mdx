...
<Steps>
  <Step title="① Connect your data">
    ...
    # Or a document (the original is stored; text extraction runs in the background)
  ...
  <Step title="② It comes back standardized">
    ...
    That's the [standardization pipeline](/en/api-reference/standardization) — deterministic name→LOINC (no LLM code-guessing) + UCUM + FHIR, on every **structured-record** write. To extract and standardize readings from a document, use [`POST /v1/extract`](/en/api-reference/extract), either with the file itself or an uploaded `file_key`.
  </Step>
...
The console Playground follows the same flow: **① Ingest data → ② Auto-standardize → ③ Agent or Answers**. Use it to upload a file, inspect extracted text or standardized records, and run either API without writing code.

<CardGroup cols={2}>
  <Card title="Playground" icon="flask" href="https://platform.mirobody.ai/developer/data">

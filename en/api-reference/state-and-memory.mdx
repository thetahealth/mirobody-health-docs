...
```bash
curl -s https://mirobody-api.thetahealth.ai/v1/responses \
...
- **Three deletion levers**: `DELETE /v1/responses/{id}` removes one entry **and retracts the memory its conversation contributed**; [`DELETE /v1/sessions/{id}`](/en/api-reference/sessions) tears down the whole journal session; [`DELETE /v1/subjects/{user}`](/en/api-reference/subjects) erases everything for the Subject.

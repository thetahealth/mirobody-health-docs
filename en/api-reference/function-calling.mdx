...
The `/v1` agent's domain tools are read-only. It also uses the deep-agents harness (`write_todos`, `task`, `eval`, filesystem tools) internally. These runs appear in the **top-level `tool_steps` extension** of the response object (never as `output` items, which official SDKs would mis-parse), and in streaming as the [`response.mirobody_tool_call` side-channel event](/en/api-reference/streaming#server-tool-side-channel).

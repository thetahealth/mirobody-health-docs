...
| Rule | Detail |
| --- | --- |
| Type | Only `"type": "function"` is supported today. Both the flat Responses form and the completions-nested `{"type":"function","function":{...}}` form are accepted. `{"type": "mcp"}` is on the roadmap — see [MCP servers](/en/api-reference/mcp-servers). |

---
title: "Configuration"
description: "Complete configuration guide for Mirobody: OAuth, AI providers, agents, MCP exposure, storage, and security."
---
...
Mirobody uses environment-specific configuration files:
...
Mirobody ships with **three** agent types:

- **DeepAgent** — single-model tool orchestration, the default; requires `OPENROUTER_API_KEY`
- **MixAgent** — two-phase fusion (capable orchestrator + cheap responder) for cost/quality balance
- **BaseAgent** — direct LLM chat without tools, low-latency Q&A

### DeepAgent Configuration

DeepAgent supports multiple LLM providers and is recommended for complex tasks. `OPENROUTER_API_KEY` is required for it to start.
...
```yaml config.localdb.yaml
# Same key conventions as DeepAgent
PROVIDERS_MIX:
  orchestrator:
    llm_type: openai
    api_key: OPENROUTER_API_KEY
...
BaseAgent is optimized for lightweight operations:

```yaml config.localdb.yaml
# API Keys for BaseAgent
GOOGLE_API_KEY: 'your_google_api_key'           # For Gemini models
MIROTHINKER_API_KEY: 'your_mirothinker_key'     # For MiroThinker

# Provider Configurations for BaseAgent
PROVIDERS_BASE:
  gemini-3-flash:
    api_key: GOOGLE_API_KEY
    model: gemini-3-flash-preview
...
<Info>
For detailed comparison of agent capabilities, see [Tools Overview](/en/tools/overview).
</Info>
...
<Steps>
  <Step title="Register as a Garmin Developer">
    1. Visit <a href="https://developer.garmin.com/">Garmin Connect Developer Portal</a>
...
<Steps>
  <Step title="Register as a Whoop Developer">
    1. Visit <a href="https://developer.whoop.com/">Whoop Developer Portal</a>
...
Mirobody supports multiple AI providers for chat functionality.
...
<Steps>
  <Step title="Get API Key">
    1. Sign up at <a href="https://platform.openai.com/">OpenAI Platform</a>
...
<Steps>
  <Step title="Get API Key">
    1. Visit <a href="https://aistudio.google.com/app/apikey">Google AI Studio</a>
...
<Steps>
  <Step title="Get OpenRouter API Key">
    1. Sign up at <a href="https://openrouter.ai/">OpenRouter</a>
...
<Steps>
  <Step title="Get API Key">
    1. Sign up at <a href="https://console.anthropic.com/">Anthropic Console</a>
...
<Info>
**`MCP_PUBLIC_URL` is the switch for HTTP Remote MCP.** Set it to a public HTTPS URL (e.g. `https://yourdomain.com` or a tunneled `https://abc123.ngrok.io`) and your MCP server becomes reachable at `<MCP_PUBLIC_URL>/mcp` for ChatGPT Apps, Claude Desktop over remote, and any other MCP client. See [ChatGPT Apps](/en/tools/chatgpt-apps).
</Info>
...
<Tabs>
  <Tab title="Google OAuth">
    ...
    Get credentials from <a href="https://console.cloud.google.com/">Google Cloud Console</a>.
  </Tab>
  ...
  <Tab title="Apple Sign In">
    ...
    Get credentials from <a href="https://developer.apple.com/">Apple Developer Portal</a>.
  </Tab>
</Tabs>
